...
public class WebServer {
    public static void main(String[] args) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(port, queueLength)) {
            ...
            System.out.println("You can access http://localhost:" + port + path + " now.");
            while (true) {
                ...
                switch (proc.getRequest().getRequestLine().split(" ")[1]) {
                    case "/create/itemid" -> proc.createItem();
                    case "/delete/itemid" -> proc.deleteItem();
                    case "/exec/params" -> proc.execParams();
                    default -> proc.process();
                }
